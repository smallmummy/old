...
run;
```
the data is like below, later we can use that data anlayzing the score distribution:
...
later we need to compare and analyze some indicator between different classes, so we need to add Mean and STD for each class.
...
There are some students whose variation of all 8 term during 2 academic years are very big, that means the score fluctuated in a wide range.

however it's necessary for us to know some featured fluctuation below:  

* go down and up with distinct fluctuation alternately.  
* go down or go down in stable pace gradually.

the above 2 featured diagram indicated the different condition: the first one means unstable score, maybe the student has the true knowledge, however can not show up firmly, or maybe the student dont have much true knowledge, just get the higher score with guessing or gambling-study. The second one means the student improve their learning setp by step, that's the good phenomenon, and vice versa.

#### 5.9.1 go down or go down in stable pace gradually

1. pre-process data set: add new indicator:  
	Add the indicator which can express totally improvement or countermarch of 8 terms during 2 academic years.  
	
	The method of calculation:  
	
	* the score of following term deduct the score of previous term, such as term2 - term1.
	* accumulate all of gap of above steps, totally 7 gaps.
	...
	then, transpose the data set using for later plot:
...
1. subsetting data:  
	getting name of top 16 students order by going-up trend:  
...
1. Plot:  
	...
	after we glimpse at the result figure, we could be aware which sub-figure is what we want easily. In this case, I spot the expectation using a red star.  
	we can check and analyze those target students carefully  with more details.


1. plot for going down parts:

	same procedures with above ones, just change the order sequence by using ascending:  
	...
	the result figure like following:
	...
	same with above, I spot target students using red star as well.  


#### 5.9.2 go down and up with distinct fluctuation alternately
	
because there were already stand deviation in the table, we can direct use them. Then we start with subsetting:  

1. subsetting data:  
	getting name of top 16 students order by STD:  
